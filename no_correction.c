...
void no_correction(WINDOW *subwin, char *str, int boolean, int function) {	
	...
	wattron(subwin, A_UNDERLINE | COLOR_PAIR(1) | A_BOLD | A_ITALIC);
	mvwprintw(subwin, 10, 0, "Enter your text here:");
	wattroff(subwin, A_UNDERLINE | COLOR_PAIR(1) | A_BOLD | A_ITALIC);
	wrefresh(subwin);
	j = 0;
	startx = 0;
	starty = 11;
	...
	if(timer != 1)
		time_taken(subwin, snt, counter,counter2,boolean, 0, n);
	wrefresh(subwin);
}
